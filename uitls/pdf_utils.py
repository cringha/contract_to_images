...
def snapshot(doc: Any, page_num: int, filename: str, info: str, pdf_filename: str | Path):
    # 2. 截取首页（第一个包含目标人员的页面）
...
def snap_pdf_all_page(pdf_filename: str | Path, info: str, cb_filename, max_pages: int=0) -> List[Path]:
    # 打开PDF文件
    # pymupdf.open
    ...
    for page_num in range(doc.page_count):
        ...
        if file_name is not None:
            snapshot(doc, page_num, file_name, info, pdf_filename)
            output_file_list.append(Path(file_name))
        else:
